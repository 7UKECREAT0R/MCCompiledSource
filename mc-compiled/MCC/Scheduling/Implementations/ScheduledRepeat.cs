...
public class ScheduledRepeat : ScheduledTask
{
    ...
    public override void Setup(TickScheduler scheduler, Executor executor)
    {
        ...
        this.thisFunction.Add(Command.Execute().IfScore(counter, new Range(0, null))
            .Run(Command.ScoreboardSubtract(counter, 1)));
        this.thisFunction.Add(Command.Execute().IfScore(counter, Range.Of(0))
            .Run(Command.Function(this.function.CommandReference)));
        this.thisFunction.Add(Command.Execute().IfScore(counter, Range.Of(0))
            .Run(Command.ScoreboardSet(counter, this.delay)));
    }
