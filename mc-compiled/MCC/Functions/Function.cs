...
public abstract class Function
{
    ...
    /// <summary>
    ///     The pattern that must be matched by <see cref="MatchParameters(Token[],string,int)" /> in order to call this
    ///     function.
    /// </summary>
    ...
    public void ProcessParameters(Token[] inputs, List<string> commandBuffer, Executor executor,
        ...
        using (sb.temps.PushTempState())
        {
            ...
            for (int i = 0; i < parameterCount; i++)
            {
                ...
                if (i >= inputsCount)
                {
                    if (parameter.optional)
                        ...
                        throw new StatementException(callingStatement,
                            $"Missing parameter '{parameter}' in '{this.Keyword}' call.");
                }
                else
                {
                    input = inputs[i];
                ...
                if (parameter.CheckInput(input) == ParameterFit.No)
                    throw new StatementException(callingStatement,
                        $"Couldn't accept input \"{input}\" for parameter '{parameter.name}'");
