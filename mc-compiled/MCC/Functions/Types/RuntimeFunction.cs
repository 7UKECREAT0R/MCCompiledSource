...
public class RuntimeFunction : Function
{
    ...
    public RuntimeFunction(Statement creationStatement, string name, string internalName, string documentation,
        IAttribute[] attributes, bool isCompilerGenerated = false)
    {
    ...
    /// </summary>
    [PublicAPI]
    public virtual Action<Executor> BlockOpenAction => e => { e.PushFile(this.file); };
    /// <summary>
    ///     The action to be run when this function's block closes.
    ///     Overridden by <see cref="AsyncFunction" /> to change this behavior.
    /// </summary>
    [PublicAPI]
    public virtual Action<Executor> BlockCloseAction => e => { e.PopFile(); };
    ...
    /// <returns>
    ///     True if the <see cref="RuntimeFunction" /> has an attribute of type <typeparamref name="T" />, otherwise
    ///     false.
    /// </returns>
    ...
    public virtual void TryReturnValue(ScoreboardValue value, Executor executor, Statement caller)
    {
        ...
        executor.AddCommands(commands, "returnValue",
            $"Returns the objective '{this.returnValue.InternalName}'. Called in a return command located in {this.file.CommandReference} line {executor.NextLineNumber}");
    }
    /// <summary>
    ///     Add commands and setup scoreboard to return a value.
    /// </summary>
    ...
    public virtual void TryReturnValue(TokenLiteral value, Statement caller, Executor executor)
    {
        ...
        executor.AddCommands(commands, "returnValue",
            $"Returns the literal '{value}'. Called in a return command located in {this.file.CommandReference} line {executor.NextLineNumber}");
    }
