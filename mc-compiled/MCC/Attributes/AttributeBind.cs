...
public class AttributeBind : IAttribute
{
    ...
    {
        return this.givenTargets == null
            ? $"bind: [{this.binding}]"
            : $"bind to ({string.Join(", ", this.givenTargets)}): [{this.binding}]";
    }
    ...
    public void OnAddedValue(ScoreboardValue value, Statement callingStatement)
    {
            ...
            case BindingType.custom_bool:
                if (value.type.TypeEnum != ScoreboardManager.ValueType.BOOL)
                    throw new StatementException(callingStatement,
                        $"Binding '{this.binding.molangQuery}' can only be applied to 'bool' values.");
                break;
            case BindingType.integer:
                if (value.type.TypeEnum != ScoreboardManager.ValueType.INT)
                    throw new StatementException(callingStatement,
                        $"Binding '{this.binding.molangQuery}' can only be applied to 'int' values.");
                break;
        ...
        foreach (string entity in targets)
        {
            ...
            string driverName = value.Name.ToLower() + "_driver_" +
                                value.Name.GetHashCode().ToString().Replace('-', '0');
            string scriptName = value.Name.ToLower() + "_controller_" +
                                value.Name.GetHashCode().ToString().Replace('-', '0');
            ...
            ControllerState[] states =
                this.binding.GetControllerStates(value, callingStatement, out string defaultState);
    ...
    {
        throw new StatementException(causingStatement, "Cannot apply attribute 'bind' to a function.");
    }
