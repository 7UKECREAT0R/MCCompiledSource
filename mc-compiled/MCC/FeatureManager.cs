...
internal static class FeatureManager
{
    ...
    /// </summary>
    internal static readonly IEnumerable<Feature> FEATURE_LIST =
        ((Feature[]) Enum.GetValues(typeof(Feature))).Where(f => f != 0);

    /// <summary>
    ///     Dictionary of features and their actions to perform on an executor when enabled.
    /// </summary>
    private static readonly Dictionary<Feature, Action<Executor>> ENABLE_ACTIONS = new()
    {
        {
