...
public class ComparisonSet : List<Comparison>
{
    ...
    {
        return this.SelectMany(comparison => comparison.GetAssertionTargets());
    }
                ...
                case TokenIdentifierValue identifierValue:
                {
                    ...
                    if (tokens.NextIs<TokenCompare>(false))
                    {
                        TokenCompare.Type comparison =
                            tokens.Next<TokenCompare>("comparison operator").GetCompareType();
                        Token b = tokens.Next();
    ...
    public void Run(Executor executor, Statement callingStatement)
    {
        ...
        {
            IEnumerable<string> partCommands =
                comparison.GetCommands(executor, callingStatement, usesElse, out bool cancel0);
            Subcommand[] localChunks =
                comparison.GetExecuteChunks(executor, callingStatement, usesElse, out bool cancel1);
...
public abstract class Comparison
{
    ...
    {
        this.inverted = invert ? !this.originallyInverted : this.originallyInverted;
    }
