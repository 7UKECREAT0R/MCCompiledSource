...
public sealed class MolangBindingInt : MolangBinding
{
        ...
        out string defaultState)
    {
        ...
        for (int i = 0; i < numberOfStates; i++)
        {
            ...
            director.transitions[i] =
                new ControllerState.Transition(state, (MolangValue) $"{this.molangQuery} == {currentState}");
            states[i + 1] = state;
        }
...
public sealed class MolangBindingFloat : MolangBinding
{
        ...
        out string defaultState)
    {
        ...
        for (int i = 0; i < numberOfStates; i++)
        {
            ...
            director.transitions[i] = new ControllerState.Transition(state,
                (MolangValue) $"{this.molangQuery} >= {lowerBound} && {this.molangQuery} < {upperBound}");
            states[i + 1] = state;
        }
        ...
        foreach (JProperty property in bindingsDict.Properties())
        {
            ...
            switch (type.ToUpper())
            {
                case "BOOL":
                    ...
                    break;
                case "INT":
                    int min = (body["min"] ?? throw new Exception($"Missing field 'min' in binding '{query}'."))
                        .Value<int>();
                    int max = (body["max"] ?? throw new Exception($"Missing field 'max' in binding '{query}'."))
                        .Value<int>();
                    binding = new MolangBindingInt(query, targets, desc, min, max);
                    break;
                case "FLOAT":
                    float minF = (body["min"] ?? throw new Exception($"Missing field 'min' in binding '{query}'."))
                        .Value<float>();
                    float maxF = (body["max"] ?? throw new Exception($"Missing field 'max' in binding '{query}'."))
                        .Value<float>();
                    float stepF = (body["step"] ?? throw new Exception($"Missing field 'step' in binding '{query}'."))
                        .Value<float>();
                    binding = new MolangBindingFloat(query, targets, desc, minF, maxF, stepF);
                    break;
                case "CUSTOM_BOOL":
                    string customQuery =
                        (body["condition"] ?? throw new Exception($"Missing field 'condition' in binding '{query}'."))
                        .Value<string>();
