...
internal class FunctionRoundRuntime : GenerativeFunction
{
        ...
        out ScoreboardValue resultValue)
    {
        ...
        output.Add(Command.Execute().IfScore(temp2, new Range(null, half - 1))
            .Run(Command.ScoreboardOpSub(resultValue, temp2)));
    }
}

internal class FunctionFloorCompiletime : CompiletimeFunction
{
    public FunctionFloorCompiletime() : base("floor", "compiletimeFloor", "decimal ?",
        "Rounds down the given value to the nearest integer.")
    {
        AddParameter(
            new CompiletimeFunctionParameter<TokenNumberLiteral>("number")
        );
...
{
    public FunctionFloorRuntime() : base("floor", "runtimeFloor", "decimal ?",
        "Rounds down the given value to the nearest integer.")
    {
        AddParameter(
            new RuntimeFunctionParameterDynamic(this, "number", "runtime_floor_num").WithAcceptedTypes(
...
{
    public FunctionCeilingCompiletime() : base("ceiling", "compiletimeCeiling", "decimal ?",
        "Rounds up the given value to the nearest integer.")
    {
        AddParameter(
            new CompiletimeFunctionParameter<TokenNumberLiteral>("number")
        );
...
{
    public FunctionCeilingRuntime() : base("ceiling", "runtimeCeiling", "decimal ?",
        "Rounds up the given value to the nearest integer.")
    {
        AddParameter(
            new RuntimeFunctionParameterDynamic(this, "number", "runtime_ceil_num").WithAcceptedTypes(
                Typedef.FIXED_DECIMAL)
        );
    }
    public override void GenerateCode(CommandFile output, int uniqueIdentifier, Executor executor, Statement statement,
