...
public static class TemporaryFilesManager
{
    ...
    private static readonly string TEMP_FOLDER_PATH =
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), ".mccompiled");
    ...
    public static string Get(PackType packType, Statement callingStatement, params string[] pathEntries)
    {
        ...
        if (!DownloadNewest(packType, pathEntries))
            throw new StatementException(callingStatement,
                $"Couldn't download file: {(packType == PackType.BehaviorPack ? "BP" : "RP")}/{string.Join("/", pathEntries)}");
