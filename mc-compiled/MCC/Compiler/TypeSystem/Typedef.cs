...
public abstract partial class Typedef
{
        ...
        Statement callingStatement)
    {
        return new StatementException(callingStatement,
            $"Literal [{literal.AsString()}] could not be converted to {value.type.TypeKeyword}");
    }
    /// <summary>
    ...
    /// <summary>
    ///     This is only used in cases where a type's internal data needs to be compared, like decimal values with mismatched
    ///     precisions.<br />
    ///     Use this as the comparison before running <see cref="CanConvertTo(Typedef)" /> and then <see cref="ConvertTo" />.
    ///     kthxbye
    /// </summary>
    ...
    /// <summary>
    ///     Setup temporary variables, return the commands to set it up, and return the JSON raw terms needed to display a
    ///     value under this type.
    ///     <br />
    ...
    /// <summary>
    ///     Returns the commands needed to subtract another value from self, given that they are both the same type and
    ///     compatible.
    ///     <code>
    ///     self -= other
    /// </code>
    ...
    /// <summary>
    ///     Returns the commands needed to multiply self by another value, given that they are both the same type and
    ///     compatible.
    ///     <code>
    ///     self *= other
    /// </code>
    ...
    /// <summary>
    ///     Returns the commands needed to get the remainder of self after division with another value, given that they are
    ///     both the same type and compatible.
    ///     <code>
    ///     self %= other
    /// </code>
    /// </summary>
