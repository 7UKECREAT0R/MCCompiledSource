...
public class Directive
{
    ...
    /// </summary>
    public bool IsPreprocessor => this.identifier.Length > 0 ? this.identifier[0].Equals('$') : false;
...
public static class Directives
{
    ...
    public static IEnumerable<Directive> PreprocessorDirectives =>
        REGISTRY.Where(directive => directive.identifier[0] == '$');
    ...
    private static void ReadJSON(JObject root)
    {
        ...
        foreach (KeyValuePair<string, JToken> directiveJSON in allDirectivesJSON)
        {
            ...
            if (body.TryGetValue("aliases", out JToken aliasesToken))
            {
                ...
                Debug.Assert(aliasesToken is JArray,
                    $"language.json/directives/{identifier}/aliases was not an array.");
            ...
            {
                Debug.Assert(patternsToken is JArray,
                    $"language.json/directives/{identifier}/patterns was not an array.");
                IEnumerable<JArray> patternsJSON = ((JArray) patternsToken)
                    .Select(token =>
                    {
                        Debug.Assert(token is JArray,
                            $"language.json/directives/{identifier}/patterns contained an item that was not an array.");
                        return (JArray) token;
                    });
            ...
            Debug.Assert(info != null,
                $"Missing implementation for: DirectiveImplementations.{_function}(executor, statement)");
            ...
            if (body.TryGetValue("attributes", out JToken attributesToken))
            {
                ...
                Debug.Assert(attributesToken is JArray,
                    $"language.json/directives/{identifier}/attributes was not an array.");
                ...
                DirectiveAttribute[] attributes = strings
                    .Select(str =>
                    {
                        if (!attributeLookup.TryGetValue(str, out DirectiveAttribute a))
                            Debug.Assert(false,
                                $"In language.json/directives/{identifier}/attributes: Attribute '{str}' is not a valid attribute.");
                        return a;
                    })
                    .ToArray();
