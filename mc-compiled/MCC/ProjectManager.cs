...
internal class ProjectManager
{
    ...
    internal void AddFile(IAddonFile file)
    {
        ...
        {
            if (this.copyFiles.Add(copyFile))
                this.files.Add(file);
            return;
        }
    ...
    private void ProcessManifests()
    {
        ...
        bool needsBehaviorManifest =
            !hasBehaviorManifest & this.files.Any(file => file.GetOutputLocation().IsBehavior());
        bool needsResourceManifest =
            !hasResourceManifest & this.files.Any(file => !file.GetOutputLocation().IsBehavior());
