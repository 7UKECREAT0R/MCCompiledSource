...
public class SoundDefinitions : IAddonFile
{
    ...
    /// <param name="soundDefinition">
    ///     When this method returns, contains the sound definition associated with the specified
    ///     key, if the key was found; otherwise, null.
    /// </param>
...
public class SoundDefinition
{
    ...
    public static SoundDefinition Parse(string name, JObject json, Statement callingStatement)
    {
        ...
        JArray soundsArray = json["sounds"] as JArray ??
                             throw new StatementException(callingStatement,
                                 $"Sound definition '{name}' missing 'sounds' array.");
        JToken categoryString = json["category"] ?? throw new StatementException(callingStatement,
            $"Sound definition '{name}' was missing 'category' string.");
        ...
        foreach (JToken token in soundsArray)
            ...
            switch (token.Type)
            {
                ...
                case JTokenType.Object:
                    ...
                    if (nameField == null)
                        throw new StatementException(callingStatement,
                            $"Sound definition '{name}' ({SoundDefinitions.FILE}) sounds array contains foreign object.");
    ...
    public JProperty ToJSON()
    {
        ...
        return new JProperty(this.CommandReference, jObject);
    }
}
