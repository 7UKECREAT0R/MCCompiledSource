...
internal class Lang : IAddonFile
{
    ...
    internal static Lang Parse(LocaleDefinition locale, string file, bool isForBehaviorPack)
    {
        ...
        foreach (string line in lines)
        {
            ...
            if (line.StartsWith("##"))
            {
                ...
                string comment = new(line[2..].SkipWhile(char.IsWhiteSpace).ToArray());
    ...
    private void FindHeader()
    {
        ...
        if (this.headerIndex == -1)
        {
            ...
            this.lines.Add(LangEntry
                .Empty()); // to prevent List::Insert() from throwing, since it can't insert at the end.
        }
        else
        {
