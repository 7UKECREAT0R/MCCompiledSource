...
public struct HasItems
{
    ...
    public static HasItems Parse(string fullSelector)
    {
        ...
        var hasitem = new HasItems(new List<HasItemEntry>());
        ...
        IEnumerable<string> parts = from part in split select part.Trim('{', '}');

        // item=goongongy,location=slot.armor.head,slot=0
        foreach (string part in parts)
        {
            string[] sections = part.Split(',');
            var entry = new HasItemEntry();
...
public struct HasItemEntry
{
    ...
    /// </summary>
    public bool IsBare =>
        !this.location.HasValue && !this.slot.HasValue && !this.data.HasValue && !this.quantity.HasValue;

    public override bool Equals(object obj)
    {
        return obj is HasItemEntry entry && this.item == entry.item && this.data == entry.data &&
               this.location == entry.location && this.slot == entry.slot &&
